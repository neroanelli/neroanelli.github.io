			...
			_this.mouseover(function(event) {  //鼠标在上边的时候停止运动
				...
				if (options.switcHover) {
					...
					for (var i = 0; i < oLen; i++) {
						options.loopNode.append("<img src='images/b"+((i+page)%oLen+1)+".jpg' name='"+(i+page)%oLen+"' alt='' />")
					};
				};
...
$(function(){
		$("#banner2").changePic({
			...
			loopNode:$("#banner2-inner2"),
			loopCircle:"-600px",
			sameNode:$("#banner2-inner2 img"),
			attrData:"name",
			leftBtn:$(".prev"),
			rightBtn:$(".next")
		});
